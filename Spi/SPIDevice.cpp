...
namespace exploringBB {
	...
	*/
	unsigned char SPIDevice::readRegister(unsigned int registerAddress) {
		unsigned char send[1], receive[2];
		unsigned char* null_return = 0x00;
		memset(send, 0, sizeof send);
		memset(receive, 0, sizeof receive);
		send[0] = (unsigned char)(0x54);
	...
	unsigned char SPIDevice::readRegisterPollarityChanged() {
		unsigned char send[1], receive[1];
		unsigned char* null_return = 0x00;
		memset(send, 0, sizeof send);
		memset(receive, 0, sizeof receive);
		send[0] = (unsigned char)(0x54);
		this->transfer(null_return, receive, 1);
		//cout << "The value that was received is: " << (int) receive[1] << endl;
		printf("0x%x\n", receive[0]);
		return receive[0];
		//this->close();
	...
	unsigned char* SPIDevice::readDevice() {
		unsigned char send[2], receive[3];
		unsigned char* ptr = receive;
		unsigned char* null_return = 0x00;
		memset(send, 0, sizeof send);
		memset(receive, 0, sizeof receive);
		//send[0] = (unsigned char)(0xff);
		//send[1] = (unsigned char)(0xff);
		this->transfer(null_return, receive, 3);
		//printf("0x%x\n", receive[0]);
		//printf("0x%x\n", receive[1]); 
		printf("0x%x\n", receive[2]);
		//cout << "The value that was received is: " << (int) (receive[0]) << endl;
		this->close();
		return ptr;
	}


	int SPIDevice::writeDevice() {
		unsigned char null_return = 0x00;
	...
	*/
	int SPIDevice::setSpeed(uint32_t speed) {
		this->speed = speed;
		//cout << SPI_IOC_RD_MAX_SPEED_HZ << endl;
	...
	*/
	...
	uint32_t SPIDevice::getSpeed() {
		return this->speed;
	}

	string SPIDevice::getFile() {
